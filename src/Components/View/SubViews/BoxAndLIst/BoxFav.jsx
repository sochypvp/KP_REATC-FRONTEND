...
const Box = ({ id, profile, name, brand, price, favId, discount, handleCloseMessage }) => {
  ...
  return (
    <article className="relative bg-gray-50 shadow-sm max-h-[300px] m-1 border p-2 flex-wrap w-[24%] max-md:w-3/6 max-sm:w-2/4 pb mb-10 pr-3 overflow-hidden">
      <Link to={"/products/" + id} key={id} className="w-full h-full">
        ...
        <button onClick={handleRemoveFav} className="btn text-danger m-1 btn-sm h-[30px] absolute bottom-1 text-lg right-14 flex items-center justify-center"><TrashIcon className="w-4 h-4 " /></button>

        <button
          onClick={handleAddToCart}
          className="btn m-1 absolute text-lg bottom-1 right-4 flex items-center justify-center"
        >
          {
            check ? (
              <Loading />
